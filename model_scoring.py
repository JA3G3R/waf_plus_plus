...
if_= IsolationForest(n_estimators=10,random_state=11)
if_.fit(features_train_get,y_get_train)
preds = if_.predict(features_test_get)
print("ERROR with IF GET model: ",mean_absolute_error(y_get_test,preds))

if_.fit(features_train_post,y_post_train)
preds = if_.predict(features_test_post)
print("ERROR with IF GET model: ",mean_absolute_error(y_post_test,preds))

gnb.fit(features_train_get,y_get_train)
preds=gnb.predict(features_test_get)
print("ERROR with GNB GET model: ",mean_absolute_error(y_get_test,preds))

gnb.fit(features_train_post,y_post_train)
preds=gnb.predict(features_test_post)
print("ERROR with GNB POST model: ",mean_absolute_error(y_post_test,preds))

lr_get.fit(features_train_get,y_get_train)
preds=lr_get.predict(features_test_get)
print("ERROR with LR GET model: ",mean_absolute_error(y_get_test,preds))
# pickle.dump(svc,open(f'models/logistic_regression_get.sav','wb'))

lr_post.fit(features_train_post,y_post_train)
preds=lr_post.predict(features_test_post)
print("ERROR with LR POST model: ",mean_absolute_error(y_post_test,preds))
# pickle.dump(svc,open(f'models/logistic_regression_get.sav','wb'))

svc.fit(features_train_post,y_post_train)
preds=svc.predict(features_test_post)
print("ERROR with SVC POST model(C=15): ",mean_absolute_error(y_post_test,preds))
# pickle.dump(svc,open(f'models/sigmoid_svc_post.sav','wb'))
...
print("ERROR with SVC GET model(C-15): ",mean_absolute_error(y_get_test,preds))
# pickle.dump(svc,open(f'models/sigmoid_svc_get.sav','wb'))

linear_svc.fit(features_train_get,y_get_train)
preds=linear_svc.predict(features_test_get)
print("ERROR with Linear SVC GET model(C-15): ",mean_absolute_error(y_get_test,preds))

linear_svc.fit(features_train_post,y_post_train)
preds=linear_svc.predict(features_test_post)
print("ERROR with Linear SVC POST model(C-15): ",mean_absolute_error(y_post_test,preds))
